...
status=$(curl --head --location -s -w "%{http_code}" -m 5 $url)
# Проверяем какие коды получили в ответ на запрос, если 200 или 302, то ищем уже текст. Если другая ошибка, то выводи ее код
